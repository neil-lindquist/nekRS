...
struct interp_data* interp_setup(nrs_t *nrs, double tol, unsigned nelm) {
  ...
  unsigned D = nrs->mesh->dim;

  struct comm gs_comm;
  comm_init(&gs_comm, rns->mesh->comm);

  // element geometry
  double elx[3] = {nrs->mesh->x, nrs->mesh->y, nrs->mesh->z};

  // element dimensions
  unsigned n1[3] = {nrs->mesh->N, nrs->mesh->N, nrs->mesh->N};

  unsigned m1[3] = {2*n1[0], 2*n1[1], 2*n2[2]};

  // used for # of cells in hash tables
  int hash_size = nelm*n1[0]*n1[1];
  if (D == 3) hash_size *= n1[2];

  void *findpts_handle = ogsFindptsSetup(gs_comm, D, elx, n1, nelm, m1, bb_tol,
